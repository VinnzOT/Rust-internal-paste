...
namespace mid_framework
{
	...
	auto add_child(int num) -> void
	{
		...
		if (num == 1)
		{
			//functions::outline_box({ position.x + 8, position.y + 30 }, { resolution.x - 17, resolution.y - 83 }, { 1, 1, 1, 1 }, 1.f);
		}
		else if (num == 2)
		{
			//functions::outline_box({ position.x + 8, position.y + 30 }, { resolution.x - 334, resolution.y - 83 }, { 1, 1, 1, 1 }, 1.f);
	...
	auto add_checkbox(std::string name, bool* value, int child) -> void
	{
		...
		if (hovered)
		{
			functions::fill_box({ cursor.x, cursor.y + 1, 14, 14 }, { 0.129f, 0.129f, 0.129f, 1 });
		}
	...
	auto add_slider_float(std::string name, float* value, float min, float max, int child) -> void
	{
		...
		const auto hovered = is_hovering({ cursor.x - 3, cursor.y + 4 }, { 310, 310 });
		const static auto max_width = 180;

		if (hovered)
		{
			functions::fill_box({ cursor.x, cursor.y + 19 - 4, max_width, 3 }, { 0.254f, 0.254f, 0.254f, 5 });
		}
		else
		{
			functions::fill_box({ cursor.x, cursor.y + 19 - 4, max_width, 3 }, { 0.196f, 0.196f, 0.196f, 5 });
		}
		...
		functions::label({ cursor.x + max_width - 2, cursor.y - 8, 150, 20 }, std::to_string(static_cast<std::int32_t>(*value)).c_str(), { 1, 1, 1, 1 }, false);
