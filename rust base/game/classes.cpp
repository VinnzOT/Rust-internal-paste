...
namespace rust
{
	...
	void PlayerEyes::rotationLook(Vector4 value)
	{
		uintptr_t entity = reinterpret_cast<uintptr_t>(this);
		if (!(entity))
			return;
	...
	void Item::Norecoil(float value)
	{
		...
		else if (item_name.find(xorstr_(L"l96.")) != std::string::npos && item_name.find(xorstr_(L"ammo.")) == std::string::npos)
		{
			memory::write<float>(recoiloverride + 0x18, 0.0f);
			memory::write<float>(recoiloverride + 0x1C, 0.0f);
			memory::write<float>(recoiloverride + 0x20, 0.0f);
		...
		else if (item_name.find(xorstr_(L"eoka.")) != std::string::npos && item_name.find(xorstr_(L"ammo.")) == std::string::npos)
		{
			memory::write<float>(recoiloverride + 0x18, 0.0f);
			memory::write<float>(recoiloverride + 0x1C, 0.0f);
			memory::write<float>(recoiloverride + 0x20, 0.0f);
	...
	bool Item::is_gun()
	{
		...
		else if (item_name.find(xorstr_(L"sniper.")) != std::string::npos && item_name.find(xorstr_(L"ammo.")) == std::string::npos)
		{
			return true;
		}
		else if (item_name.find(xorstr_(L"bolt.")) != std::string::npos && item_name.find(xorstr_(L"ammo.")) == std::string::npos)
		{
			return true;
		}
		else if (item_name.find(xorstr_(L"smg.")) != std::string::npos && item_name.find(xorstr_(L"ammo.")) == std::string::npos)
	...
	void BasePlayer::longeye()
	{
		...
		memory::write<Vector3>(eye + 0x38, Vector3(0.0f, 2.0f, 0.0f));
	}
